...
def ratings(request, id):
    classes = Lecture.objects.get(id = id)
    texts = Comments.objects.filter(lecture=classes)
    if request.user.is_authenticated:
        if request.method == "POST":
            comments = Comments()
            comments.lecture = classes
            comments.body = request.POST['review']
            comments.save()
            return render(request, 'ratings.html',{'lecture' : classes, 'comment' : texts }) 
        else:
            return render(request, 'ratings.html',{'lecture' : classes, 'comment' : texts }) 
